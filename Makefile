...
########################
APP_NAME=idp
SERVER_HOST_PORT=8082
SERVER_ADDR=:9000
POSTGRES_USER=postgres
POSTGRES_APP_USER=idp_app_user
POSTGRES_ADMIN_USER=idp_app_administrator
POSTGRES_PASSWORD=olvidar
POSTGRES_DB=canary_health
POSTGRES_HOST=idp-pgdb
POSTGRES_HOST_PORT=5434
PROJECT_PATH=github.com/canary-health/idp
...
ifndef TAG
	...
	MIGRATION_DB_URL=$(shell echo $(shell aws ssm get-parameter --name /${TAG}/idp-server/MIGRATION_DB_URL --query "Parameter"."Value" --with-decryption))
endif
...
## Docker commands
######
.PHONY: build-image build-server-image build-test-image run-container
...
migrate-up: ;
	docker run --rm -v $(MIGRATION_PATH)/database/migrations/:/migrations/ --network host migrate/migrate -path=/migrations/ -database=$(MIGRATION_DB_URL) up

migrate-down: ;
	docker run --rm -v $(MIGRATION_PATH)/database/migrations/:/migrations/ --network host migrate/migrate -path=/migrations/ -database=$(MIGRATION_DB_URL) down

## Sets clean version in schema_migrations table. 
## Takes param v={{version}} e.g. v=400
migrate-force: ;
	docker run --rm -v $$GOPATH/src/$(PROJECT_PATH)/database/migrations/:/migrations/ --network host migrate/migrate -path=/migrations/ -database $(MIGRATION_DB_URL) force $(v)
...
pbjs: ; $(info $(M) Generating typescript_client files ...)
	go generate ./api/typescript_client/gen.go;
...
rm-pb: ; $(info $(M) Removing generated protobuf files... )
	for tw in ./internal/**/*.twirp.go; do \
		$(RM) $$tw; \
		echo Removed: $$tw; \
	done
	for pb in ./internal/**/*.pb.go; do \
		$(RM) $$pb; \
		echo Removed: $$pb; \
	done
	for sw in ./api/swagger-spec/*.swagger.json; do \
		$(RM) $$sw; \
		echo Removed: $$sw; \
	done
	for ts in ./api/typescript_client/lib/*; do \
		$(RM) $$ts; \
		echo Removed: $$ts; \
	done
